...
export function OrderAll({ orders = [], onOrderUpdate }) {
      ...
      {orders.map((order) => (
        <div key={order._id} className='flex flex-col rounded-lg border bg-white p-4 shadow-sm'>
          <div className='mx-2 mb-2 flex flex-row justify-between'>
            ...
            <p className={`text-lg font-bold ${order.status === 'Delivered' ? 'text-green-500' : order.status === 'Shipped' ? 'text-yellow-500' : order.status === 'Pending' ? 'text-yellow-500' : order.status === 'Cancelled' ? 'text-red-500' : ''}`}>{STATUS_TH[order.status] || order.status}</p>
          </div>
          {/* สินค้าในออเดอร์ */}
